...
//     // 특정 시행년도의 일정 조회
//     List<ExamEntity> findByImplYear(String implYear);
...
//     List<ExamEntity> findActiveSchedules(@Param("currentDate") LocalDate currentDate, Pageable pageable);
...
//     List<ExamEntity> findUpcomingSchedules(@Param("currentDate") LocalDate currentDate, Pageable pageable);
...
//     List<ExamEntity> findSchedulesByYears(@Param("thisYear") String thisYear, @Param("nextYear") String nextYear);

//     // [기능 5] 특정 연도의 모든 시험 일정 조회 (캘린더용, N+1 방지)
//     @Query("SELECT s FROM ScheduleEntity s JOIN FETCH s.certificate c WHERE s.implYear = :year")
//     List<ExamEntity> findSchedulesByYear(@Param("year") String year);
...
public interface ExamRepository extends JpaRepository<ExamEntity, Long>, JpaSpecificationExecutor<ExamEntity> {
    ...
    // ExamRepository.java 에 추가
boolean existsByCertificateAndImplYearAndImplSeqAndType(
    CertificateEntity certificate, String implYear, Integer implSeq, String type
);
}